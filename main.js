...
menus.forEach((menu) =>
  menu.addEventListener("click", (event) => {categoryKey = event.target.textContent.toLowerCase(); getNewsByCategory(event)}));
sideMenu.forEach((menu)=> menu.addEventListener("click", (event) => {categoryKey = event.target.textContent.toLowerCase(); getNewsByCategory(event)}));
...
function searchBoxWorks() {
  ...
  // 닫혀있으면 입력창 열기
  if (inputArea.style.display === "inline") {     // 써치 인풋 입력창 보이는 상태인가?    
    if (searchInput.value == "") {      // 써치 인풋 비어있는지 체크
      inputArea.style.display = "none"; // 입력창이 비어있으면 보이는 상태를 안보이게
    } else {      // 써치 인풋에 내용이 있으면 q에 인풋밸류 넣고 겟뉴스
      qKey = searchInput.value;            
      getNewsByCategory();
...
const getNewsByCategory = async (event) => {     
  ...
  console.log("ddd by cate", categoryKey, newsList);
  render();
};

// 이미지URL 유효성 검사 - 김진한님 코드
...
const render = () => {
  qKey !== "" ? searchAlarm() : document.getElementById("search-result").innerHTML = ""; 
  const newsHTML = newsList
    .map((news) => {
      // null값 체크 파트 - 김진한님 코드
      const imageUrl = news.urlToImage ? news.urlToImage : "imgs/no-image.png";
      // url 유효성 체크(ture or false값 들어감)
      const validatedImage = validateImageUrl(imageUrl);
      // validatedImage가 true이면 imageUrl
      // false이면 noimage
      return `<div class="row news">
    <div class="col-lg-4">
      <img class="news-img-size" src="${validatedImage ? imageUrl : "imgs/no-image.png"}"/>
    </div>    
    <div class="col-lg-8">
      <h2>${news.title.length>40?news.title.substring(0,40)+"...":news.title}</h2>  
      <p>${
